...
- **The bonus was the live chat demo, not a scroll-driven story.** I considered a scroll-triggered narrative sequence (messages appearing as you scroll, to *imply* real-time behavior) versus embedding the actual `MessageBubble`/`TypingIndicator` components in a small working sandbox you can type into. I picked the second: it's provably real — the component rendering your message is the same component rendering messages in `/chat` — rather than an animation that merely looks like the feature. The assignment's bonus bar is "genuinely original," and a real, working, typed-into sandbox clears that bar in a way a generic testimonial carousel or feature-icon grid wouldn't.
...
- Driving ad hoc Playwright scripts against a running dev server to verify the login → dashboard → landing-page flow and the live chat demo actually work in a real browser, not just compile.
